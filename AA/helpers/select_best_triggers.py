...
def rank_triggers(df, group, sort_cols, sort_order, n_triggers):
    ...
    weights = [
        10 ** (3 * (len(sort_cols) - i - 1)) for i in range(len(sort_cols))
    ]  # e.g. [1e6, 1e3, 1] for 3 cols
    df["rank_score"] = sum(
    ...
    df_selected = (
        df_top.groupby(group, group_keys=False)
        .apply(tie_breaker)
        .reset_index(drop=True)
    )
...
def standard_trigger_selection(
    df, group=["index", "category", "issue_ready"], n_triggers=2
):
...
def fbeta_trigger_selection(
    df, group=["index", "category", "issue_ready"], beta=1, n_triggers=2
):
...
def fbeta_score(x, beta=1):
    return (
        (1 + beta**2)
        * x["TP"]
        / ((1 + beta**2) * x["TP"] + x["FP"] + (beta**2) * x["FN"])
    )
...
# %%
c = fbeta_trigger_selection(
    valid_df, ["index", "category", "issue_ready"], 1, n_triggers=1
)
