...
import fsspec
import re
from config.params import S3_OPS_DATA_PATH
...
def pivot_triggers_pilots_data(df):
    ...
    cols_to_keep = ['district', 'category', 'index', 'issue_ready', 'issue_set', 
                    'prob_ready', 'prob_set', 'trigger_ready', 'trigger_set']
    df_subset = df[cols_to_keep].copy()
    
    # Pivot the data to create long format
    df_ready = df_subset[['district', 'category', 'index', 'issue_ready', 'prob_ready', 'trigger_ready']].copy()
    ...
    df_set = df_subset[['district', 'category', 'index', 'issue_set', 'prob_set', 'trigger_set']].copy()
    df_set['phase'] = 'set'
    ...
    # Reorder columns
    result_df = result_df[['district', 'category', 'issue', 'index', 'prob', 'trigger', 'phase']]
...
def validate_probabilities_match(prob_df, trigger_df):
    ...
    logging.info(f"Expected missing rows (issue > {max_issue}): {len(expected_missing)}")
    ...
    if len(both_datasets) > 0:
        both_datasets['prob_match'] = abs(both_datasets['prob_trigger'] - both_datasets['prob_prob']) < 1e-10
        mismatches = both_datasets[~both_datasets['prob_match']]
        mismatches["impacts_programme"] = (
            (mismatches["trigger"]>mismatches["prob_trigger"])!=(mismatches["trigger"]>mismatches["prob_prob"])
        )
    else:
        mismatches = pd.DataFrame()
    
    # Find unexpected missing rows (should have matches but don't)
    unexpected_missing = merged[merged['_merge'] == 'left_only'][merge_cols + ['prob_trigger', 'phase']]
    ...
    return {
        'matches': matches,
        'mismatches': mismatches[merge_cols + ['trigger', 'prob_trigger', 'prob_prob', 'phase','impacts_programme']] if len(mismatches) > 0 else pd.DataFrame(),
        'missing_in_prob': unexpected_missing,
        'expected_missing': expected_missing[merge_cols + ['prob', 'phase']] if len(expected_missing) > 0 else pd.DataFrame()
    }
...
def main(iso):
    ...
    logging.info(f"Found {len(prob_files)} probability files: {[f.split('/')[-1] for f in prob_files]}")
    ...
    logging.info(f"Loaded {len(triggers_raw_df)} rows from {triggers_file.split('/')[-1]}")
    ...
    if validation_results['matches']:
        ...
        if len(validation_results['mismatches']) > 0:
            logging.error(f"🔍 {len(validation_results['mismatches'])} probability mismatches found:")
            logging.error(f"\n{validation_results['mismatches']}")
            
        if len(validation_results['missing_in_prob']) > 0:
            logging.error(f"📊 {len(validation_results['missing_in_prob'])} unexpected missing rows in trigger data:")
    ...
    if len(validation_results['expected_missing']) > 0:
        logging.info(f"💡 {len(validation_results['expected_missing'])} rows expected to be missing (issue > max available):")
        logging.info(f"📊 These have issue values {validation_results['expected_missing']['issue'].unique()}")
