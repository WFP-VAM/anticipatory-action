...
area = AnalysisArea.from_admin_boundaries(iso3='TZA', admin_level=2, resolution=0.25, datetime_range=DATES)
area.geobox
...
area_lta = AnalysisArea.from_admin_boundaries(iso3='TZA', admin_level=2, resolution=0.25, datetime_range="1970-01-01/1970-12-31")
obs_lta = area_lta.get_dataset(['CHIRPS', 'R3H_DEKAD_LTA'],                      
    load_config={
        "gridded_load_kwargs": {
             "resampling": "bilinear",
        }
...
auc = xr.open_zarr(f"ad-hoc-evaluations/tanzania/outputs/SPI {period}/auc_{period.lower()}_issue{iss}.zarr").histogram_observations_forecasts
auc_bc = xr.open_zarr(f"ad-hoc-evaluations/tanzania/outputs/SPI {period}/auc_bc_{period.lower()}_issue{iss}.zarr").histogram_observations_forecasts
...
for c in auc.category:
    if not os.path.exists(f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI {period}/{c.values}"):
        os.mkdir(f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI {period}/{c.values}")
        
    write_cog(auc.sel(category=c.values), f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI {period}/{c.values}/auc_{str(c.values).lower()}_{period.lower()}_issue{iss}.tif", overwrite=True)
    write_cog(auc_bc.sel(category=c), f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI {period}/{c.values}/auc_bc_{str(c.values).lower()}_{period.lower()}_issue{iss}.tif", overwrite=True)
    
    figure = auc.sel(category=c.values).hip.viz.map(title=f'AUC {c.values} {period}', cmap='RdYlGn')
    figure.savefig(f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI {period}/{c.values}/auc_{str(c.values).lower()}_{period.lower()}_issue{iss}.png")
    
    figure = auc_bc.sel(category=c.values).hip.viz.map(title=f'AUC BC {c.values} {period}', cmap='RdYlGn')
    figure.savefig(f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI {period}/{c.values}/auc_bc_{str(c.values).lower()}_{period.lower()}_issue{iss}.png")
# -

# Without bias correction
cat = 'Moderate'
auc.sel(category=cat).hip.viz.map(title=f'AUC {cat} {period}', cmap='RdYlGn') #.plot.imshow()
...
figure.savefig(f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI OND/rfh_lta_ond.png")

figure = obs_jfm.drop_vars('time').hip.viz.map(title='RFH LTA JFM', cmap=rxs.cmap)
figure.savefig(f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI JFM/rfh_lta_jfm.png")

figure

figure = obs_mam.drop_vars('time').hip.viz.map(title='RFH LTA MAM', cmap=rxs.cmap)
figure.savefig(f"ad-hoc-evaluations/tanzania/outputs/rasters/SPI MAM/rfh_lta_mam.png")
figure
