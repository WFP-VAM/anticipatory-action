...
for (d, c, w), case in df_merged.groupby(["district", "category", "window"]):
    ...
    df_merged.loc[
        (df_merged.district == d)
        & (df_merged.window == w)
        & (df_merged["category"] == c),
        "state",
    ] = state
# -

outcomes_new = pd.DataFrame(
    columns=[
        "W1-Leve",
        "W1-Moderado",
        "W1-Severo",
    ...
    index=df_merged["district"].sort_values().unique(),
)
...
for (d, c, w), case in ref.groupby(["district", "category", "window"]):
    ...
    ref.loc[
        (ref.district == d) & (ref.window == w) & (ref["category"] == c), "state"
    ] = state

outcomes_old = pd.DataFrame(
    columns=[
        "W1-Leve",
        "W1-Moderado",
        "W1-Severo",
...
def compare_new_old_outcomes(old, new):
    ...
    colors = {
        "now set": "darkgreen",
        "both set": "mediumseagreen",
        "both NA": "mediumseagreen",
        "both ready": "mediumseagreen",
        "was set before": "coral",
        "missing so far": "burlywood",
        "not complete": "burlywood",
    }
    return comp.style.map(
        lambda val: "background-color: {}".format(colors.get(val, ""))
    )
